...
def get_tags_from_md(folder_path):
    """
    (вспомогательное) Находит в файле md теги и печатает их. Или выводит сообщение об отсутствии тегов.
    Можно изменять конечную часть процедуры, чтобы получать разный вывод.
    :param file_path:
    :param relative_path:
    :return:
    """
            ...
            if filename.endswith(".md"):
                ...
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    if not tags_list:  # НЕТ ТЕГОВ
                        ...
                        # Только список тегов
                        # print(tags_list)
                        # Собрать список всех тегов во всех файлах
                        all_tags.extend(tags_list)
                        # pass
    if all_tags:
        set_tags = set(all_tags)  # Удаляем дубли
        sorted_all_tags = sorted(set_tags)  # set не поддерживает сортировку
...
def create_tag_folders(tags, destination_path):
    ...
    for tag in tags:
        ...
        if tag.startswith('#'):
            folder_name = tag.lstrip('#').replace('-', ' ')
        else:
            folder_name = tag
...
def delete_attach_folder(source_dir, dest_dir):
    """
    После переноса вложений attache папка удаляется.
    :param source_dir:
    :param dest_dir:
    :return:
    """
    print(f"  delete_attach_folder for {source_dir} and {dest_dir}")
    ...
    if os.path.exists(source_dir) and os.path.isdir(source_dir):
        ...
        if files_in_source_dir:
            print(f"  В папке ещё есть файлы")
            # source_dir has files, so copy them to folder2
            if not os.path.exists(dest_dir):
                os.makedirs(dest_dir)
            for file in files_in_source_dir:
                file_path = os.path.join(source_dir, file)
                print(f"  Move attach {file_path} to {dest_dir}")
                shutil.move(file_path, dest_dir)

            # Delete source_dir
            print(f"  Delete dir {source_dir}")
            os.rmdir(source_dir)
        else:
            # source_dir is empty, so just delete it
            print(f"  Delete EMPTY dir {source_dir}")
            os.rmdir(source_dir)
    else:
...
def remove_parentheses(data_path, dry=True):
    """
    Удаление скобок в названиях md файлов, в именах ресурсных подпапок и в линках в заметке.
    :param data_path:
    :param dry:
    :return:
    """
...
if __name__ == '__main__':
    ...
    # Вывод тегов в разных видах или md файлов без тегов.
    # get_tags_from_md(DATA_PATH)
